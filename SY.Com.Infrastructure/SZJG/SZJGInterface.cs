...
namespace SY.Com.Infrastructure
{
    ...
    public class SZJGInterface
    {
        ...
        public SZJG_Return_Model send(bool isFile = false,string filepath = "")
        {
            try {
                ...
                if (isFile)
                {
                    ...
                    List<FormItemModel> datafiles = new List<FormItemModel>();
                    FormItemModel tempfile = new FormItemModel();
                    tempfile.Key = "";
                    tempfile.FileName = "测试图片.png";
                    MemoryStream file = new MemoryStream(jmfilebytes);
                    tempfile.FileContent = file;
                    datafiles.Add(tempfile);
                    result = WebClient.PostForm(SdkRequestUtils.httpUrl + apipath, datafiles, headers, null, null, null);
                    file.Close();
                }
                else {
                    List<PostDateClass> datas = new List<PostDateClass>();
                    datas.Add(new PostDateClass("channelId", requestCommonDTO.getChannelId()));
                    datas.Add(new PostDateClass("requestId", requestCommonDTO.getRequestId()));
                    datas.Add(new PostDateClass("encodeKey", requestCommonDTO.getEncodeKey()));
                    datas.Add(new PostDateClass("sign", requestCommonDTO.getSign()));
                    datas.Add(new PostDateClass("signMethod", requestCommonDTO.getSignMethod()));
                    datas.Add(new PostDateClass("timestamp", requestCommonDTO.getTimestamp()));
                    datas.Add(new PostDateClass("version", requestCommonDTO.getVersion()));
                    datas.Add(new PostDateClass("encodeData", requestCommonDTO.getEncodeData()));
                    ...
                    result = WebClient.postMessage(SdkRequestUtils.httpUrl + apipath, datas, headers, false);
        ...
        {
            SZJG_Return_Model wuha = new SZJG_Return_Model();
            return wuha;
        }
